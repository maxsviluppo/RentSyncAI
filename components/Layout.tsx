...
const SidebarItem: React.FC<{ icon: React.ReactNode; label: string; active: boolean; onClick: () => void }> = ({ icon, label, active, onClick }) => (
  <button
    onClick={onClick}
    className={`group relative w-full flex items-center justify-between px-4 py-3.5 rounded-xl transition-all duration-300 ease-in-out ${active
    ...
    <div className="flex h-screen overflow-hidden bg-[#F8FAFC]">
      ...
      <div className="w-72 bg-[#020617] flex flex-col h-full flex-shrink-0 border-r border-slate-900 shadow-2xl relative z-20">
        ...
        <div className="p-4 border-t border-slate-800/60 bg-[#020617]">
          ...
          />
          <button className="w-full mt-2 flex items-center gap-3 px-4 py-3.5 text-red-400/80 hover:bg-red-500/10 hover:text-red-400 rounded-xl transition-all duration-200 group">
