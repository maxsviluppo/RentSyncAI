...
import { CarStatus, AIRecommendation, Contract, DriverProfile, Agent, Car } from '../types';
...
const MobileContract: React.FC<{ currentAgent: Agent }> = ({ currentAgent }) => {
  ...
  return (
    <div className="h-full overflow-y-auto pb-24 p-5 scrollbar-hide">
            ...
            <div className="space-y-3 text-sm text-slate-600 mb-6">
                ...
                <div className="bg-indigo-50 p-3 rounded-lg text-indigo-900 font-bold text-center text-lg mt-4">
                  Totale: € {
                      ...
                      Math.max(1, Math.ceil((new Date(dates.end).getTime() - new Date(dates.start).getTime()) / (1000 * 3600 * 24)))
                  }
                </div>
            </div>
...
const MobileAddClient = ({onClose}: {onClose: () => void}) => {
    ...
    return (
      <div className="absolute inset-0 bg-white z-50 p-5 pt-12 animate-in slide-in-from-bottom flex flex-col">
...
const MobileQuote = () => {
  ...
  const handleAi = async () => {
    ...
    try {
      ...
      const r = await recommendCar(availableFleet, profile);
      setRecs(r);
      setStep(2);
...
const AgentMobileApp: React.FC<AgentMobileAppProps> = ({ sessionAgentId, onLogout }) => {
  ...
  }, [agents, isLoggedIn, sessionAgentId]);
  ...
  if (!isLoggedIn || !currentAgent) {
    return (
      <div className="flex items-center justify-center h-full bg-slate-100 p-4 font-sans overflow-hidden">
         <div className="w-full max-w-[340px] h-[75vh] max-h-[700px] bg-black rounded-[40px] border-[10px] border-slate-900 shadow-2xl overflow-hidden relative flex flex-col items-center justify-center bg-gradient-to-br from-indigo-950 via-slate-900 to-black text-white">
            <div className="absolute top-3 left-1/2 -translate-x-1/2 w-[100px] h-[30px] bg-black rounded-full z-20"></div>
            
            <div className="w-20 h-20 bg-white/10 rounded-3xl flex items-center justify-center backdrop-blur-md mb-8 shadow-inner ring-1 ring-white/20">
               <Smartphone className="w-8 h-8 text-white" />
            </div>
            
            <h2 className="text-2xl font-bold mb-2 tracking-tight">Agent<span className="text-indigo-400">Pro</span></h2>
            <p className="text-indigo-200/60 mb-8 text-center px-8 text-xs">Inserisci il tuo nickname aziendale.</p>
            ...
            <div className="w-full px-8 space-y-4">
               <div>
                  ...
                  <input 
                    type="text" 
                    className="w-full bg-white/5 border border-white/10 rounded-2xl p-3 text-white placeholder-white/20 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all text-center text-base"
                    placeholder="es. demo"
                    value={agentNickname}
               ...
               <button 
                 onClick={handleLogin}
                 className="w-full bg-indigo-500 text-white font-bold py-3 rounded-2xl hover:bg-indigo-400 transition-all shadow-lg shadow-indigo-500/30 flex items-center justify-center gap-2 mt-2"
               >
                 <LogIn className="w-4 h-4" /> Accedi
               </button>
            </div>
            
            <p className="absolute bottom-6 text-white/20 text-[10px]">RentSync AI • v2.0</p>
         </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex items-center justify-center h-full bg-slate-200 p-4 font-sans overflow-hidden">
        {/* Realistic Phone Frame */}
        <div className="w-full max-w-[360px] h-[85vh] max-h-[740px] bg-white rounded-[40px] border-[10px] border-[#1e1e1e] shadow-2xl overflow-hidden relative flex flex-col ring-4 ring-slate-300 transition-all duration-300">
            
            {/* Side Buttons (Visual Only) */}
            <div className="absolute top-24 -left-[14px] w-[4px] h-[26px] bg-[#1e1e1e] rounded-l-md"></div>
            <div className="absolute top-36 -left-[14px] w-[4px] h-[46px] bg-[#1e1e1e] rounded-l-md"></div>
            <div className="absolute top-52 -left-[14px] w-[4px] h-[46px] bg-[#1e1e1e] rounded-l-md"></div>
            <div className="absolute top-40 -right-[14px] w-[4px] h-[70px] bg-[#1e1e1e] rounded-r-md"></div>

            {/* Dynamic Island Area */}
            <div className="absolute top-0 w-full h-[50px] z-30 flex justify-center pointer-events-none">
                 <div className="w-[110px] h-[32px] bg-black mt-3 rounded-[20px] flex items-center justify-center gap-3 px-2">
                    {/* Fake Camera/Sensors */}
                    <div className="w-10 h-10 rounded-full absolute right-2"></div> 
                 </div>
            </div>
            
            {/* Status Bar */}
            <div className="h-[48px] w-full bg-white flex justify-between px-8 pt-4 text-xs font-bold text-black z-20">
                <span>9:41</span>
            ...
            <div className="bg-white px-5 pb-3 flex justify-between items-center z-10 sticky top-0">
               <div>
                  <h1 className="text-xl font-bold text-slate-900 tracking-tight">Agent<span className="text-indigo-600">Pro</span></h1>
                  <p className="text-[10px] text-slate-400">Ciao, {currentAgent.name.split(' ')[0]}</p>
               </div>
               <div className="w-9 h-9 bg-indigo-50 rounded-full flex items-center justify-center text-indigo-700 font-bold text-sm">
            ...
            <div className="flex-1 bg-[#F2F4F7] overflow-hidden relative rounded-t-[30px] shadow-[0_-10px_40px_rgba(0,0,0,0.05)]">
               ...
               {/* --- TAB: HOME / FLEET --- */}
               {activeTab === 'home' && (
                  <div className="h-full overflow-y-auto pb-24 p-5 space-y-4 scrollbar-hide">
                     
                     {/* Quick Actions */}
                     <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                        <button onClick={() => setShowAddClient(true)} className="flex-shrink-0 bg-slate-900 text-white px-4 py-2.5 rounded-2xl text-xs font-bold flex items-center gap-2 shadow-lg shadow-slate-900/20">
                           <Plus className="w-3 h-3" /> Nuovo Cliente
                        </button>
                        <button onClick={() => setActiveTab('contract')} className="flex-shrink-0 bg-white text-slate-700 px-4 py-2.5 rounded-2xl text-xs font-bold flex items-center gap-2 shadow-sm border border-slate-100">
                           <PenTool className="w-3 h-3 text-indigo-600" /> Contratto
                        </button>
                     </div>

                     {/* Fleet Feed */}
                     <h3 className="font-bold text-slate-800 text-base px-1">Disponibili Ora</h3>
                     {fleet.map(car => (
                        <div key={car.id} className="bg-white rounded-3xl p-3 shadow-sm border border-slate-100 flex flex-col gap-2">
                           <div className="relative">
                              <img src={car.image} className="w-full h-28 object-cover rounded-2xl" alt="car" />
                           ...
                           <div className="flex justify-between items-end">
                              <div>
                                 ...
                                 <p className="text-[10px] text-slate-500 mt-0.5">{car.category} • {car.plate}</p>
                              </div>
               ...
               {/* --- TAB: PROFILE --- */}
               {activeTab === 'profile' && (
                  <div className="p-5 overflow-y-auto pb-24 h-full scrollbar-hide">
                     <div className="text-center pt-4">
                         <div className="w-20 h-20 bg-gradient-to-tr from-indigo-500 to-purple-500 rounded-full flex items-center justify-center text-white font-bold text-2xl mx-auto mb-4 shadow-xl shadow-indigo-200">
                            {currentAgent.nickname.charAt(0).toUpperCase()}
                         </div>
                         <h3 className="font-bold text-xl text-slate-900">{currentAgent.name}</h3>
                         <p className="text-slate-500 text-xs mb-6">{currentAgent.region} • ID: {currentAgent.id}</p>
                     </div>

                     {/* QR Code Business Card */}
                     <div className="bg-white rounded-3xl p-5 shadow-lg border border-slate-100 mb-6 text-center relative overflow-hidden">
                         <div className="absolute top-0 left-0 w-full h-1.5 bg-gradient-to-r from-indigo-500 to-purple-500"></div>
                         <h4 className="font-bold text-slate-900 mb-1 flex items-center justify-center gap-2 text-sm"><QrCode className="w-3 h-3"/> Accesso Rapido Agente</h4>
                         <p className="text-[10px] text-slate-500 mb-4 px-2">Scansiona questo QR Code per accedere automaticamente all'App Mobile senza login.</p>
                         ...
                         <div className="flex gap-2">
                             <button 
                                onClick={handleShare}
                                className="flex-1 bg-slate-900 text-white py-2.5 rounded-xl font-bold text-xs flex items-center justify-center gap-2 hover:bg-slate-800 transition-colors"
                             >
                                 <Share2 className="w-3 h-3"/> Invia Link
                             </button>
                             <button 
                                onClick={() => {
                     ...
                     <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 mb-6 grid grid-cols-2 gap-3">
                        <div className="text-center">
                           <div className="text-xl font-bold text-slate-900">{currentAgent.activeClients}</div>
                           <div className="text-[10px] text-slate-500 uppercase font-bold">Clienti</div>
                        </div>
                        <div className="text-center">
                           <div className="text-xl font-bold text-slate-900">{currentAgent.commissionRate}%</div>
                           <div className="text-[10px] text-slate-500 uppercase font-bold">Comm.</div>
                        </div>
                     </div>

                     <div className="bg-gradient-to-r from-green-500 to-emerald-600 text-white p-5 rounded-2xl shadow-lg shadow-green-200 mb-6">
                        <p className="text-green-100 text-xs font-medium mb-1 flex items-center justify-center gap-1"><DollarSign className="w-3 h-3"/>Guadagno Totale</p>
                        <p className="text-3xl font-bold">€ {myEarnings.toLocaleString()}</p>
                     </div>

                     <button onClick={handleLogout} className="w-full py-3 text-red-500 bg-white border border-red-100 rounded-2xl font-bold text-xs hover:bg-red-50 transition-colors">Disconnetti</button>
                  </div>
               )}
            ...
            {/* Bottom Floating Nav */}
            <div className="absolute bottom-6 left-4 right-4 h-[65px] bg-black/90 backdrop-blur-xl rounded-[32px] flex justify-around items-center px-1 z-40 shadow-2xl">
               <button onClick={() => setActiveTab('home')} className={`p-2.5 rounded-full transition-all ${activeTab === 'home' ? 'bg-white/20 text-white' : 'text-white/50'}`}>
                  <Home className="w-5 h-5" />
               </button>
               <button onClick={() => setActiveTab('quote')} className={`p-2.5 rounded-full transition-all ${activeTab === 'quote' ? 'bg-white/20 text-white' : 'text-white/50'}`}>
                  <Sparkles className="w-5 h-5" />
               </button>
               <button onClick={() => setActiveTab('contract')} className={`p-2.5 rounded-full transition-all ${activeTab === 'contract' ? 'bg-white/20 text-white' : 'text-white/50'}`}>
                  <PenTool className="w-5 h-5" />
               </button>
               <button onClick={() => setActiveTab('profile')} className={`p-2.5 rounded-full transition-all ${activeTab === 'profile' ? 'bg-white/20 text-white' : 'text-white/50'}`}>
                  <User className="w-5 h-5" />
               </button>
            </div>
            
            {/* Home Indicator */}
            <div className="absolute bottom-2 left-1/2 -translate-x-1/2 w-[110px] h-[4px] bg-black rounded-full z-50"></div>
        </div>
    </div>
  );
};
